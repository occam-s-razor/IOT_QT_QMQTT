...
ProcessImage::ProcessImage()
{
    ...
    pp = (unsigned char *)malloc(640 * 480 * 3 * sizeof(char));/*QWidget::width()*QWidget::height()*///每一帧RGB数据
    //painter = new QPainter(this);//drow tool
   // frame = new QImage(pp,640,452,QImage::Format_RGB888);//QImage类主要用于I/O和直接逐像素访问,display image...采集的图像应该是RGB888格式
   // label = new QLabel();
  ...
  //  QMessageBox::warning(this,tr("error"),tr("init failed"),QMessageBox::Yes);
    ...
    rs = vd->start_capturing();//驱动程序开始采集
    if(-1==rs)
    {
   ...
   // timer->start(30);//30ms,timeout

   // QHBoxLayout *hLayout = new QHBoxLayout();//布局
...
void ProcessImage::paintEvent(uchar *ppout)
{
    rs = vd->get_frame((void **)&p,(size_t*)&len);//读取一帧数据
    ...
    convert_yuv_to_rgb_buffer(p,pp,640,480);/*QWidget::width(),QWidget::height()*///yuv格式转rgb格式
    ppout=pp;
    //frame->loadFromData((uchar *)pp,/*len*/640 * 480 * 3 * sizeof(char));

    //label->setPixmap(QPixmap::fromImage(*frame,Qt::AutoColor));
    // label->show();
   // rs = vd->unget_frame();//将缓冲区从新排入队列
}
